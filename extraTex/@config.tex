...
\usepackage{siunitx} %标准单位包
\usepackage{circledsteps} %圆形序号
...
\geometry{left=3.20cm,right=3.20cm,top=2.54cm,bottom=2.54cm} % 页边距（对齐 2026 Word 模板 PDF v2）
\CJKsetecglue{\hskip 0.15em plus 0.05em minus 0.05em} % 数字与 CJK 字符的间距（对齐 Word 2026）
% left=3.20cm,right=3.20cm

%%%————————————————————————————————颜色
% 说明：在 XeLaTeX + xcolor 组合下，RGB(0,112,192) 在 PDF 中会被量化为 (0,111,192)；
...
\addto\captionsenglish{
    ...
    \renewcommand{\refname}{\sihao \templatefont \bfseries \leftline{参考文献}} %四号字，楷体，居左
...
\else
  ...
  \setCJKmonofont[Path=./fonts/, Extension=.ttf, AutoFakeBold=3]{Kaiti} % 定义 CJK 等宽字体（避免 xeCJK tt 字体警告）
  \setmainfont[BoldFont=Times New Roman, AutoFakeBold=5]{Times New Roman} %使用MacOS自带的Times New Roman，可以自由使用英文斜体和粗体。
  \xeCJKsetup{PunctStyle=quanjiao} % 强制中文标点符号使用中文字体
\fi
\newfontfamily\titlenf{Times New Roman}[BoldFont=Times New Roman, AutoFakeBold=3] % 1. 定义标题专用英文/数字字体（Times New Roman）

%%%————————————————————————————————行距
% Word 2026 模板：行距为“固定值 22 磅”
% 说明：不使用 baselinestretch 乘法去逼近，直接把默认字号(12pt)的 baselineskip 固定为 22pt
\renewcommand{\baselinestretch}{1.0}
\AtBeginDocument{\fontsize{12pt}{20pt}\selectfont\frenchspacing} %原始为22pt
\setlength{\parindent}{0pt} % 保持模板：main.tex 中已有 \hspace*{2em} 控制提纲提示语缩进
\setlength{\parskip}{0pt} % 段间距：对齐模板的默认视觉效果
\hfuzz=20pt\relax % 避免示例图宽度导致的 Overfull \hbox 警告
\XeTeXlinebreaklocale "zh" % 中文断行

% Word 空白段落的"视觉高度"对齐：在已有 \parskip 的情况下补足到 1 行行距
\newcommand{\NSFCBlankPara}{\vspace*{\dimexpr\baselineskip-\parskip\relax}}

% 正文段首缩进：仅在 extraTex 正文中启用，避免与 main.tex 的 \hspace*{2em}/\linebreak{} 叠加
\newcommand{\NSFCBodyText}{%
  \setlength{\parindent}{2em}%
  % 正文段间距：让段间距与行间距观感一致（紧凑），因此在正文区块内关闭段后距。
...
\usepackage{caption}
\captionsetup[figure]{name={图},font={normalsize,stretch=1.25},labelsep=period, singlelinecheck=off, justification=centering} 
...
\captionsetup[table]{name={表},font={normalsize,stretch=1.25},labelsep=period, singlelinecheck=off, justification=centering} 
...
\titleformat{\subsubsection}
  ...
  {\textnormal{\templatefont \titlenf \arabic{subsection}.\arabic{subsubsection}}} % label
  {0.5em} % separation
  {}   % before-code
...
\titleformat{\subsubsubsection}
  ...
  {\titlenf(\arabic{subsubsubsection})~~} % label
  {0.5pt} % separation
  {\titlenf}   % before-code
\makeatletter
...
\titlespacing*{\section}
  ...
  {2pt plus 0pt minus 0pt} % Space before the title
  {1.5pt} % Space after the title（用于减小与 subsection 的距离；避免依赖 \parskip）

% Define line stretch for subsection
\titlespacing*{\subsection}
  {0pt} % Left indentation
  {1.5pt plus 0pt minus 0pt} % Space before the title
  {5pt} % Space after the title（模拟 Word“段后 7.8pt”，且不依赖全局 \parskip）

% Define line stretch for subsubsection
\titlespacing*{\subsubsection}
  {0pt} % Left indentation
  {3pt plus 0pt minus 0pt} % Space before the title
  {2pt plus 0pt minus 0pt} % Space after the title
...
\renewenvironment{thebibliography}[1]{%
  ...
  % 参考文献标题外观保持与 release 一致（走 \section 的 MsBlue 配色），但不走 \section* 以避免 spacing 差异
  \noindent{\color{MsBlue}\refname}\par%
...
% 参考文献中的链接字体用的是texttt
\urlstyle{same} % URL 使用正文同款字体（对齐 Word）
...
% \setlength{\textfloatsep}{1cm plus 1.0pt minus 2.0pt} % 浮动体（如figure）之间的垂直间距
%\setlength{\intextsep}{8pt plus 2.0pt minus 2.0pt} % 浮动体（如figure）与文字的垂直间距 0.5cm
\makeatother
...
% 自定义第4级小标题
% 说明：避免 \textcircled 触发 OMS/TimesNewRoman 等字体警告，使用 pifont 的圈号字符
